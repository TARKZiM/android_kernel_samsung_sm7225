...
static bool ts_read_coord(struct zt_ts_info *info)
{
	...
	u16 status_data;
	u16 pocket_data;
	u8 coordinate_event = false;

	info->proximity_only = false;
	...
	if (info->fod_enable) {
		...
		if (read_data(info->client, ZT_FOD_STATUS_REG,
			...
			input_err(true, &client->dev, "%s: Failed to read Touch FOD info\n", __func__);
			return false;
		}
		...
		if (info->fod_info_vi_data_len > 0) {
			if (read_data(info->client, ZT_VI_STATUS_REG,
				...
				input_err(true, &client->dev, "%s: Failed to read Touch VI Data\n", __func__);
				return false;
			}
		}
	...
	if (info->touch_info[0].byte00.value.eid == COORDINATE_EVENT) {
		...
		if (info->touched_finger_num > 0) {
			if (read_data(info->client, ZT_POINT_STATUS_REG1, (u8 *)(&info->touch_info[1]),
				...
				input_err(true, &client->dev, "%s: Failed to read touched point info\n", __func__);
				return false;
			}
		}
		...
		for (i = 0; i < info->cap_info.multi_fingers; i++) {
			u16 prox_data = 0;
			u8 ttype = (info->touch_info[i].byte06.value.touch_type23 << 2)
					| (info->touch_info[i].byte07.value.touch_type01);
			u8 tstatus = info->touch_info[i].byte00.value.touch_status;

			if (tstatus != FINGER_NONE)
				coordinate_event = true;

			if ((ttype == TOUCH_PROXIMITY) && (info->pdata->support_ear_detect)) {
				if (read_data(info->client, ZT_PROXIMITY_DETECT, (u8 *)&prox_data, 2) < 0) {
					input_err(true, &client->dev,
							"%s: fail to read proximity detect reg\n", __func__);
					return false;
				}

				if (coordinate_event == false)
					info->proximity_only = true;
		...
		u16 cur_version, u16 cur_minor_version, u16 cur_reg_version, u16 cur_hw_id)
{
	...
	if (info->pdata->force_update_fac_fw_ver > 0 &&
			cur_reg_version >= info->pdata->force_update_fac_fw_ver) {
		input_info(true, &info->client->dev, "%s: [FAC] force update cause 0x%02x >= 0x%02x\n",
				__func__, cur_reg_version, info->pdata->force_update_fac_fw_ver);
		return true;
	}
#endif
...
static void zt_set_fod_rect(struct zt_ts_info *info)
{
	write_cmd(info->client, 0x0A);
...
static bool mini_init_touch(struct zt_ts_info *info)
{
	...
	if (info->fod_enable) {
		input_info(true, &info->client->dev, "%s: fod_mode_set:0x%02X\n", __func__, info->fod_mode_set);
		if (write_reg(client, REG_FOD_MODE_SET, info->fod_mode_set) != I2C_SUCCESS)
			input_err(true, &client->dev, "%s, fail fod mode set\n", __func__);
	}
...
#else
				input_info(true, &client->dev, "[RA] tID:%d loc:%s dd:%d,%d mc:%d tc:%d p:%d\n",
						i, location,
						info->cur_coord[i].x - info->pressed_x[i],
						info->cur_coord[i].y - info->pressed_y[i],
						info->move_count[i], info->finger_cnt1,
...
static irqreturn_t zt_touch_work(int irq, void *data)
{
	...
	if (info->sleep_mode) {
		...
		ret = wait_for_completion_interruptible_timeout(&info->resume_done, msecs_to_jiffies(1000));
		if (ret == 0) {
		...
		if ((info->cur_coord[i].touch_status == FINGER_PRESS || info->cur_coord[i].touch_status == FINGER_MOVE)) {
			...
			if ((info->cur_coord[i].touch_status == FINGER_PRESS) && (info->cur_coord[i].touch_status != info->old_coord[i].touch_status)) {
				...
				input_info(true, &client->dev, "[P] tID:%d,%d x:%d y:%d z:%d(st:%d) max:%d major:%d minor:%d loc:%s tc:%d touch_type:%x noise:%x\n",
						...
						x, y, z, st, sen_max, info_major_w,
						info_minor_w, location, info->finger_cnt1,
						info->cur_coord[i].ttype, info->cur_coord[i].noise);
#else
				input_info(true, &client->dev, "[P] tID:%d,%d z:%d(st:%d) max:%d major:%d minor:%d loc:%s tc:%d touch_type:%x noise:%x\n",
						i, (info->input_dev->mt->trkid - 1) & TRKID_MAX,
						z, st, sen_max, info_major_w,
						info_minor_w, location, info->finger_cnt1,
						info->cur_coord[i].ttype, info->cur_coord[i].noise);
#endif
			} else if (info->cur_coord[i].touch_status == FINGER_MOVE) {
				info->move_count[i]++;
				if (info->old_coord[i].touch_status == FINGER_NONE
						|| info->old_coord[i].touch_status == FINGER_RELEASE) {
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
					input_info(true, &client->dev, "[M] tID:%d,%d x:%d y:%d z:%d(st:%d) max:%d major:%d minor:%d loc:%s tc:%d touch_type:%x noise:%x\n",
							i, (info->input_dev->mt->trkid - 1) & TRKID_MAX,
							x, y, z, st, sen_max, info_major_w,
							info_minor_w, location, info->finger_cnt1,
							info->cur_coord[i].ttype, info->cur_coord[i].noise);
#else
					input_info(true, &client->dev, "[M] tID:%d,%d z:%d(st:%d) max:%d major:%d minor:%d loc:%s tc:%d touch_type:%x noise:%x\n",
							i, (info->input_dev->mt->trkid - 1) & TRKID_MAX,
							z, st, sen_max, info_major_w,
							info_minor_w, location, info->finger_cnt1,
							info->cur_coord[i].ttype, info->cur_coord[i].noise);
...
#else
			input_info(true, &client->dev,
					"[R] tID:%d loc:%s dd:%d,%d mc:%d tc:%d p:%d\n",
					i, location,
					info->cur_coord[i].x - info->pressed_x[i],
					info->cur_coord[i].y - info->pressed_y[i],
					info->move_count[i], info->finger_cnt1,
...
static void zt_ts_close(struct input_dev *dev)
{
	...
	if (((info->spay_enable || info->aod_enable || info->aot_enable || info->singletap_enable))
		...
		for (i = 0; i < 3; i++) {
			...
			write_cmd(info->client, ZT_SLEEP_CMD);
			zt_delay(10);
			read_data(info->client, 0x01FB, (u8 *)&rbuff, 2);
			input_info(true, &info->client->dev, "%s: enter sleep mode %d, retry=%d\n", __func__, rbuff, i);
			if (rbuff == 1)
				break;
		}
...
static void run_cnd_read_all(void *device_data)
{
	...
	char buff[16] = { 0 };
	char *all_cmdbuff;
	int buff_size = info->cap_info.x_node_num * info->cap_info.y_node_num * 6;
	s32 i, j;
	int ret;
...
static void run_ssr_read_all(void *device_data)
{
	...
	sec_cmd_set_cmd_result(sec, all_cmdbuff,
			strnlen(all_cmdbuff, total_node * 6));
	sec->cmd_state = SEC_CMD_STATUS_OK;
	kfree(all_cmdbuff);
...
NG:
	...
	/* the condition 'mis_cal_data == 0xFD' cannot be true
...
static void run_cs_raw_read_all(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", "OK");
	sec_cmd_set_cmd_result(sec, all_cmdbuff, strnlen(all_cmdbuff, buff_size));
	sec->cmd_state = SEC_CMD_STATUS_OK;
	kfree(all_cmdbuff);
...
static int zt_power_ctrl(void *data, bool on)
{
	...
	if (!pdata->gpio_ldo_en) {
		regulator_dvdd = regulator_get(&info->client->dev, "tsp_dvdd");
		if (IS_ERR_OR_NULL(regulator_dvdd)) {
			input_err(true, dev, "%s: Failed to get dvdd\n", __func__);
			return PTR_ERR(regulator_dvdd);
		}
	}
	regulator_avdd = regulator_get(&info->client->dev, "tsp_avdd");
	if (IS_ERR_OR_NULL(regulator_avdd)) {
		input_err(true, dev, "%s: Failed to get avdd\n", __func__);
		if (!pdata->gpio_ldo_en)
		...
		struct zt_ts_platform_data *pdata)
{
	...
	ret = of_get_named_gpio(np, "zinitix,irq_gpio", 0);
	if (ret < 0) {
	 ...
	 */
#ifdef CONFIG_SEC_FACTORY
	of_property_read_u32(np, "zinitix,force_update_fac_fw_ver", &pdata->force_update_fac_fw_ver);
	of_property_read_string(np, "zinitix,fac_firmware_name", &pdata->firmware_name);
	input_info(true, dev, "%s: [FAC] use fac_firmware: %s, force update ver: 0x%02x\n",
			__func__, pdata->firmware_name, pdata->force_update_fac_fw_ver);
...
void zt_print_info(struct zt_ts_info *info)
{
	...
	input_info(true, &info->client->dev,
			"tc:%d noise:%s(%d) cover:%d lp:(%x) fod:%d ED:%d mode:0x%04X // v:%04X C%02XT%04X.%4s%s // #%d %d\n",
			info->finger_cnt1, info->noise_flag > 0 ? "ON":"OFF", info->noise_flag, info->flip_cover_flag,
			info->lpm_mode, info->fod_mode_set, info->ed_enable, info->m_optional_mode.select_mode,
			fw_version,
#ifdef TCLM_CONCEPT
			info->tdata->nvdata.cal_count, info->tdata->nvdata.tune_fix_ver,
